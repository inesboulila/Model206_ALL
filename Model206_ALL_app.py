...
def conservation_label(val) -> str:
    """Convert float conservation value to display label."""
    if val is None or (isinstance(val, float) and np.isnan(val)):
        return "Unknown"
    try:
        return CONS_LABELS.get(int(float(val)), f"{int(float(val))} — Unknown")
    except (TypeError, ValueError):
        return "Unknown"

def conservation_to_float(val) -> float:
    """
    Safely convert family_conservation to float for the model.
    Returns NaN if missing — LightGBM will rely on other features.
    """
    if val is None or (isinstance(val, float) and np.isnan(val)):
        return np.nan
    try:
...
# ── miRNA name normalization and lookup ───────────────────────
...
def resolve_input(user_input: str) -> dict | None:
    """
    Accepts full miRNA name, short name (no prefix), or accession number.
    Returns the full info dict or None if not found.
    The dict contains: seed_family, family_conservation, mirbase_accession, microrna_group_simplified
    """
    s = user_input.strip()

    # 1. Exact miRNA name match
    if s in mirna_lookup:
        return mirna_lookup[s]

    # 2. Accession number match (e.g. MIMAT0000646)
    if s in accession_lookup:
        return accession_lookup[s]

    # 3. Fuzzy: normalize input and compare against all stored names
    ...
    if predict_btn:
        if not mirna_input.strip():
            ...
            if info is not None:
                ...
                raw_cons   = info.get('family_conservation')  # float: 2.0, 1.0, 0.0, -1.0, or NaN

                # Determine if seed family is real
                if raw_family and str(raw_family) not in ('not_broadly_conserved', 'not_found', 'nan', ''):
                ...
                # Convert for display and for model
                family_conservation_num   = conservation_to_float(raw_cons)
                family_conservation_label = conservation_label(raw_cons)

                st.info(
                    f"**Seed family:** {seed_family if not (isinstance(seed_family, float) and np.isnan(seed_family)) else 'Unknown'}  \n"
            ...
            # Build input row — all numeric columns must be float
            ...
            input_df = pd.DataFrame([{
                ...
                'family_conservation': family_conservation_num,
            }])

            # Enforce numeric dtypes — prevents "object dtype" error in LightGBM
